...
Ask any LLM "what time is it?" and it guesses. Ask "what did we discuss 3 hours ago?" and it can't — a message from 5 minutes ago looks identical to one from yesterday. Context is flat without a clock.

## What `since` does

`@since_time` wraps your chat function with a real clock — timestamps every message, tracks session duration, detects gaps, and tells the model the current time so it never has to guess.
...
# Every message timestamped automatically
...
Gap: 2m between messages — welcome back!
╰ Stale: "config.py" — invalidated, 14m old
```
...
Files change between agent turns. `since` catches that — stamps file reads with mtime + content hash, and surfaces staleness when the file changes.
...
if check_and_invalidate("config.py", store, "session_1"):
    ...
    stamp_file_read("config.py", store, "session_1")  # re-stamp
```

No daemon, no polling, no watcher. Just mtime + content hash at the next turn.
